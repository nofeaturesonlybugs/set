...
func TestBoundMapping_fromSQLH1(t *testing.T) { // TODO+NB Update this test for both BoundMapping+PreparedMapping
	chk := assert.New(t)
	...
	b, err := m.Bind(&Address{})
	chk.NoError(err)

	fields, err := b.Fields([]string{"pk"}, nil)
	chk.NoError(err)
	chk.NotEmpty(fields)

	fields, err = b.Fields([]string{"pk", "created_tmz"}, nil)
	chk.NoError(err)
	chk.NotEmpty(fields)
}
